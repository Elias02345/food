...
import type { MacroTarget, PlannerEntry, Recipe, RecipeCategory, ShoppingItem } from '../types'
import { categoryIcons, days, emptyNutrition, macroFields, type Page } from '../appShared'
import { CategoryVisual, EmptyState, ProgressRing, RecipeCard, Toggle } from '../components/RecipeUI'

export function Planner({ entries, setEntries, target, onOpen, addRecipeToShopping }: { entries: PlannerEntry[]; setEntries: (value: PlannerEntry[] | ((current: PlannerEntry[]) => PlannerEntry[])) => void; target: MacroTarget; onOpen: (recipe: Recipe, scale?: number) => void; addRecipeToShopping: (recipe: Recipe, scale?: number) => void }) {
  const slots: RecipeCategory[] = ['Frühstück', 'Mittagessen', 'Pre-Workout', 'Abendessen', 'Snack']
  const autoPlan = () => {
    const allocation: Record<RecipeCategory, number> = { Frühstück: 0.22, Mittagessen: 0.28, 'Pre-Workout': 0.14, Abendessen: 0.28, Snack: 0.08 }
